...
export class ConfigService {
  ...
  readonly syncWorkerIntervalMs = Number(process.env.SYNC_WORKER_INTERVAL_MS ?? '5000');
  readonly syncMaxAttempts = Number(process.env.SYNC_MAX_ATTEMPTS ?? '5');
  readonly idempotencyTtlSec = Number(process.env.IDEMPOTENCY_TTL_SEC ?? '86400');

  validate(): void {
    if (!Number.isFinite(this.syncWorkerIntervalMs) || this.syncWorkerIntervalMs <= 0) {
    ...
    if (!Number.isFinite(this.idempotencyTtlSec) || this.idempotencyTtlSec <= 0) {
